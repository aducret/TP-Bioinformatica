...
function constructFormatParams() {
    ...
    var formatOrg = getUrlCompForEntryField($("FormatForm").FORMAT_ORGANISM);
    if (formatOrg != "" && formatOrg.indexOf("taxid") != -1) {
...
function createAlignAnchor() 
{
    ...
    if (anchorName) {
        ...
        var alignSeq = "";
        if (locParts.length == 6 || locParts.length == 1) {//#157734238_1569_22_1971_7367080_7369033 or #157734238
            alignSeq = locParts[0];
        }
...
function initAdvancedView()
{
    ...
    jQuery("#clkey").find("a.graphSeq").each(function (index) {        
...
function modifySubmitResultsParams(submitElem) 
{
    ...
    if (!found) {
        $("results").appendChild(submitElem);
    }
}
...
function openerCheck(event) 
{
    ...
    if (gi != "") {    
        DisplayAlignFromGraphics(gi, event)        
    }
}
...
function ReadGraphAlinRequest(event) {
    ...
    rp.onSuccess = function (obj) {
        ...
        jQuery("#grapArea").html(obj.responseText, {
            'widgets': ['ncbipopper', 'ncbitoggler'],
            'configs': {
                'ncbipopper': { hasArrow: true, arrowDirection: 'top' }
            }
        ...
        jQuery("#grBlastHits").find("a.jig-ncbipopper").each(function (index) {
            jQuery.ui.jig.scan(this);
        });
    };
    rp.onError = function (obj) {
